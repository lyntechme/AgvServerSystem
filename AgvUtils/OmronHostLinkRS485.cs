...
{
    /// <summary>
    /// HostLine标准fins通信协议读取数据(使用了串口转以太网的MOXA W2150A模块，RS485)
    /// </summary>
    public class OmronHostLinkRS485 : PlcConnect
    {
        #region 变量
        UdpClient _udp;
        ...
        public OmronHostLinkRS485(int localPort)
        {
        ...
        public Byte[] WReadAgv(int plcUnitNum, string strAgvControlCmd, string strAgvRamCmd, int origin, int length, string strIP, int port)
        {
            ...
            int recDataLength = 27 + length * 4 + cmdStr.Length;
        ...
        public Byte[] BReadAgv(int plcUnitNum, string strAgvControlCmd, string strAgvRamCmd, int origin, byte bit, int length, string strIP, int port)
        {
            ...
            cmdStr += CheckHelper.AsciiXorToString(cmdStr, 0) + "*\r";
            int recDataLength = 27 + length * 2 + cmdStr.Length;
            Byte[] data = RecFins(cmdStr, strIP, port, recDataLength);
            return data;
        }
        ...
        private Byte[] RecFins(string recCmd, string ipadress, int port,int recDataLength)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string[] rec = recStr.Split(new char[] { '*','\r' }, StringSplitOptions.RemoveEmptyEntries);
                    if (rec.Length == 2)
                    {
                        if (rec[1].Substring(rec[1].Length - 2, 2) != CheckHelper.AsciiXorToString(rec[1].Substring(0, rec[1].Length - 2), 0))  //校验判断
                        {
                            return error;
                        }
                        if (rec[1][0] != '@')  //标识符判断
                        ...
                        if (rec[1].Substring(19, 4) != "0000")  //通讯是否正确判断
                        {
                            return error;
                        }
                        string data = rec[1].Substring(23, rec[1].Length - 25);  //提取反馈数据
                        if (data.Length % 2 != 0)
                        {
                            return error;
                        }
        ...
        private bool SendFins(string sendCmd, string ipadress, int port)
        {
            try
            {
                int recDataLength = 27 + sendCmd.Length;
                ...
                else
                {
                    ...
                    if (rec.Length == 2)
                    {
                        if (rec[1].Substring(rec[1].Length - 2, 2) != CheckHelper.AsciiXorToString(rec[1].Substring(0, rec[1].Length - 2), 0))
                        {
                            return false;
                        }
                        if (rec[1].Substring(rec[1].Length - 6, 4) != "0000")
                        {
                            return false;
                        }
                        return true;
